...
| ウィジェット名 | 主な機能・役割 |
| :--- | :--- |
| **System Settings** | システム共通のAPIキー、AIモデルの選択、およびロールごとの権限（Permissions）を一元管理します。 |
...
* **複数ロール (Multi-Role) のアサイン**:
  ユーザーに対して `admin`（管理者）、`manager`（マネージャー）、`user`（一般）、`its`（ITサポート）など、複数のロールを同時に割り当てることが可能です。新設された `manager` ロールは、`action:manage_assistant_rules` アクション権限を標準で付与されており、自身を含むアシスタントの予定調整ルール・AIプロンプトの編集を行えます（一般 `user` にはこの権限がありません）。
