...
- `[✅]` **TC-P5-07: AWSライクなマルチロールのUI付与テスト**
  - **前提**: `admin` アカウントでログインしていること。
  - **手順**: System Settings > Users タブを開き、テストユーザーに対してドロップダウンから「ITS」や「Researcher」を追加し、タグ（チップ）として複数付与されていることを確認後、ブラウザをリロードする。
  - **期待結果**: ユーザーのロールが `its,researcher` のようにカンマ区切りで保存され、リロード後も付与したタグ（チップ）が保持されていること。
