...
- `[N/A]` **TC-SKILL-09**: ユーザーがチャットで要求した際、インストールされている外部SkillのAPI機能（Tools）がGeminiに動的に渡され、適切に外部APIが呼び出されること。
...
- `[N/A]` **TC-SKILL-10**: MacOSUIから外部のSkill APIエンドポイントにリクエストを投げる際、リクエストヘッダに有効な署名付き `JWT` が自動的に付与されていること。
  - **確認方法**: 外部API側（モックサーバー等）で受信したリクエストのヘッダをダンプし、`Authorization: Bearer <JWT>` が含まれていることを確認する。また、そのJWTをデコードし、正しい `issuer` や `scope` が含まれているかをチェックする。
- `[N/A]` **TC-SKILL-11**: 不正なスコープや権限外の操作をSkillが要求した場合、ZTAポリシーゲートウェイによってアクセスが拒否（403 Forbidden等）されること。
  - **確認方法**: Iframeウィジェット内から、意図的に権限を持たないMacOSUIの内部API（例: システム設定変更APIなど）に対してフェッチリクエストを送信する。ブラウザの開発者ツール（Networkタブ）で、レスポンスが `403 Forbidden` となり、アクセスがブロックされることを確認する。
